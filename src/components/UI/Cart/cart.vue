<template>
  <div v-if="bodyItem === 'cart'" class="cart">
    <div class="cart__top">
      <div class="cart__back" @click="nuller">
        <svg
    ...
    <div class="cart__body">
      <div class="cart__items" v-if="cartItems !== null">
        <CartItem
      ...
      <div class="cart__items empty" v-else>Корзина пуста</div>
    </div>
  </div>
  <div v-else-if="bodyItem === 'table'" class="cart">
    <div class="cart__top">
      <div class="cart__back" @click="nuller">
        <svg
          xmlns="http://www.w3.org/2000/svg"
      ...
      <h3 class="cart__title">Выбор стола</h3>
    </div>
    <div class="cart__body">
      <div class="cart__items">
        <div><p>Список столов</p></div>
        ...
        <div class="cart__input">
          <div>Введите номер стола:</div>
          <input
            style="padding: 10px; margin-top: 10px; font-size: 15px"
            type="text"
            name="tableID"
            id="id"
          />
  ...
  <div v-else-if="bodyItem === 'payment'" class="payment">
    <div class="payment__top">
      <div class="payment__back" @click="bodyItem = 'cart'">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          xmlns:xlink="http://www.w3.org/1999/xlink"
          width="25px"
    ...
    <div class="payment__body">
      <div class="payment__card">
        <div class="payment__cardTop">
          <h3>Реквизиты карты</h3>
          <div class="payment__iconList">
            <img
              src="../../../static/icons/visa-logo-svgrepo-com.svg"
              alt="visa"
            />
            <img src="../../../static/icons/mc_symbol.svg" alt="mastercard" />
          </div>
        </div>
        <div class="payment__cardInfo">
          <div class="payment__cardInput">
            <label>Номер карты</label>
            <input type="text" />
          </div>
          <div class="payment__cardInput date">
            <label>Дата</label>
            <div class="date-inputs">
              <input type="number" id="date" min="0" max="12" />
              <input type="number" id="date" min="0" max="99" />
            </div>
            <div class="cvv-input">
              <label>CVV</label>
      ...
      <div class="payment__reserve">
        <button
          class="resto-button"
          @click="clicker('finish')"
          v-if="cartItems !== null"
        >
          Оплатить
        </button>
  ...
  <div v-else class="payment">
    <div class="payment__top">
      <h3 class="payment__title">Успешно</h3>
    </div>
    <button class="resto-button" @click="nuller">Вернуться на главную</button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    clicker(_page) {
      ...
      var uniques = this.cartItems.unique(); // result = [1,3,4,2,8]
      console.log(uniques);
    },
  },
  created() {
  ...
  async beforeMount() {
    axios
      .get(backDomain + "/api/table/getAll", {
        headers: {
          "Content-Type": "application/json",
      ...
      .then((res) => {
        ...
        this.tablesLength = res.data.length;
      })
      .catch((error) => {
        console.log(error);
      });
